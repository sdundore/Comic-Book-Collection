...
public class JdbcUserDao implements UserDao {
    ...
    public User saveUser(String userName, String password, String role, String email) {
        ...
        String saltString = new String(Base64.encode(salt));
        long newId = jdbcTemplate.queryForObject(
                "INSERT INTO users(username, password, salt, role, email) VALUES (?, ?, ?, ?, ?) RETURNING id", Long.class,
                userName, hashedPassword, saltString, role, email);
    ...
    public void updateUser(User user) {
    	String sqlUpdate = "UPDATE users SET username = ?, email = ?, password = ?, role = ? WHERE id = ? ";
    	
    	jdbcTemplate.update(sqlUpdate, user.getUsername(), user.getEmail(), user.getPassword(),
